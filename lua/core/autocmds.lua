...
vim.api.nvim_create_autocmd('FileType', {
  ...
  callback = function()
    ...
    local scratch_dir_local = vim.fn.stdpath 'data' .. '/scratch'
    if bufname:match(vim.pesc(scratch_dir_local)) then
      return
    end
    ...
    local lines = vim.api.nvim_buf_get_lines(0, 0, math.min(50, line_count), false)
    for _, line in ipairs(lines) do
      if line:match '^# %%%%' then
        -- Use pcall to safely attempt MoltenInit
